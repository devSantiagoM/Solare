...
async function fetchNews() {
    ...
    try {
        ...
        // Map Supabase data to our internal format
        allNews = data.map((post) => ({
            id: post.id,
            title: post.title,
            excerpt: post.excerpt,
            category: post.category || 'coleccion',
            date: post.published_at,
            image: post.featured_image || 'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?q=80&w=800&auto=format&fit=crop',
            badge: isNew(post.published_at) ? 'Nuevo' : '',
            featured: false
        }));
...
function filterNews(category) {
    if (category === 'all') {
        ...
        filteredNews = allNews.filter(article => article.category === category);
    }
...
function renderNews() {
    ...
    if (articlesToShow.length === 0) {
        elements.newsGrid.innerHTML = '<p style="grid-column: 1/-1; text-align: center;">No hay artículos en esta categoría.</p>';
        return;
    }
...
function createArticleElement(article) {
    ...
    // Update link to point to post.html
    if (link) {
...
// Social media posts data (Static)
const socialPosts = [
    {
        id: 1,
