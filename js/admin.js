...
(function () {
  ...
  if (btnAddFaqCategory) {
    btnAddFaqCategory.addEventListener('click', () => {
      editingCategoryId = null; // Reusing or create new var? Let's use a new one or be careful.
      // Actually, I defined editingFaqId but not editingFaqCategoryId.
      // Let's use editingCategoryId for generic or define a new one.
      // I'll define editingFaqCategoryId in the scope if needed, or just use a local var if I can.
      // But I need it for submit.
      // I'll add it to the state vars at the top later or just assume I can add it here.
      // Wait, I can't easily add to top. I'll add it here.
  ...
  let editingFaqCategoryId = null;

  function openFaqCategoryModal(categoryId = null) {
    editingFaqCategoryId = categoryId;
  ...
  if (faqCategoryForm) {
    faqCategoryForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        let error;
        if (editingFaqCategoryId) {
          ({ error } = await window.supabase
            .from('faq_categories')
            .update(data)
            .eq('id', editingFaqCategoryId));
        } else {
          ({ error } = await window.supabase
            .from('faq_categories')
            .insert([data]));
        ...
        showToast(editingFaqCategoryId ? 'Categoría actualizada' : 'Categoría creada', 'success');
        closeFaqCategoryModal();
        await loadFaqCategories();
        // Reload FAQs to update category names if needed
        await loadFAQs();
  ...
  if (faqForm) {
    faqForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        showToast(editingFaqId ? 'Pregunta actualizada' : 'Pregunta creada', 'success');
        closeFaqModal();
        await loadFAQs();
  ...
  window.admin.editFaqCategory = async (id) => {
    try {
  ...
  window.admin.deleteFaqCategory = async (id) => {
  ...
  window.admin.editFaq = (id) => {
    openFaqModal(id);
  };

  window.admin.deleteFaq = async (id) => {
